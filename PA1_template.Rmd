Reproducible Research Assignment by Roberto Tan
========================================================
...
```{r}
#setwd("C:/Users/roberto.tan/Downloads/MOOC/5. Reproducible Research/Assignment 1")
...
library(ggplot2)
qplot(steps, data=actsum2, main='Histogram for Number of Steps per Day', binwidth=500)
```
...
Note that there are days/intervals with missing values (currently coded as NA). This is likely to allow for bias into some calculations or summaries of the data. Care must be taken in the analysis.
...
*My strategy is a computation with 5-minute intervals mean.*
...
qplot(steps, data=dfsum2, main='Histogram Using Imputed Values', binwidth=500)
...
*The median values do not differ very much and the mean values are identical. Thus it is highly likely that the inputs into the dataset could be mean values.*
